...
class BaseEditor:
    ...
    def train(self, loader: DataLoader):
        ...
        for tuples in tqdm(loader, desc = "Train", ncols = 100):
            ...
            if self.config.editor.name == 'horse' and 'unrel_tuples' in tuples:
                loc_losses = []
            ...
            print(f"gen_loss: {np.mean(gen_losses):.4f}, loc_loss: {np.mean(loc_losses):.4f}")
    ...
    def valid(self, loader: DataLoader):
        ...
        for tuples in tqdm(loader, desc = "Valid", ncols = 100):
            ...
            print(f"ES: {np.mean(edit_succs):.4f}, GS: {np.mean(gen_succs):.4f}, LS: {np.mean(loc_succs):.4f}")
